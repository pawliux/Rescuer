    ...
    def analyze_array(self, split_array, path):
        ...
        for i in range(len(split_array)):
            ...
            if i == len(split_array) - 1:
                # populating checklist widget
                checklist_path = checklist_path + re.sub("\W", "", split_array[i])
                APP.checklist.hlist.add(checklist_path, text=split_array[i])
                APP.checklist.setstatus(checklist_path, 'on')
                # then we set cursor to the file path on the system for later reference, r segment is removed
                cursor[split_array[i]] = RescueFile(path)
                ...
                if split_array[i] not in cursor:
                    ...
                    checklist_path = checklist_path + re.sub("\W", "", split_array[i]) + '.'
                    APP.checklist.hlist.add(checklist_path[:-1], text=split_array[i])
                    APP.checklist.setstatus(checklist_path[:-1], 'on')

                # if dictionary type is created then cursor is set one step deeper in FILES
                cursor = cursor[split_array[i]]

    def analyze_array_old(self, split_array, path):
        cursor = self.FILES
        for i in range(len(split_array)):
            # if we looped to the last segment of the path (filename)
            if i == len(split_array) - 1:
...
class AppGUI(tk.Frame):
    ...
    def __init__(self, parent,):
        ...
        self.checklist = tk.CheckList(parent, width=650)
        self.checklist.grid(row=5, column=1, columnspan=3)
        # self.checklist.hlist.add("CL1", text="checklist1")
        # self.checklist.hlist.add("CL1.Item1", text="subitem1")
        # self.checklist.hlist.add("CL2", text="checklist2")
        # self.checklist.hlist.add("CL2.Item1", text="subitem1")
        # self.checklist.hlist.add("CL2.Another", text="deeper1")
        # self.checklist.hlist.add("CL2.Another.Item1", text="even deeper1")
        # self.checklist.hlist.add("CL2.Another.Item1.Deep1", text="even even deeper1")
        # self.checklist.hlist.add("CL3", text="checklist2")
        # self.checklist.hlist.add("CL3.Item1", text="subitem1")
        # self.checklist.hlist.add("CL3.Another", text="deeper1")
        # self.checklist.hlist.add("CL3.Another.Item1", text="even deeper1")
        # self.checklist.hlist.add("CL3.Another.Item1.Deep1", text="even even deeper1")
        # self.checklist.setstatus("CL2", "on")
        # self.checklist.setstatus("CL2.Item1", "on")
        # self.checklist.setstatus("CL1", "off")
